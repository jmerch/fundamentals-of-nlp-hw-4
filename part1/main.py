...
def create_augmented_dataloader(args, dataset):
    ...
    print("unaugmented_dataset_size", len(dataset["train"]))
    sample = dataset["train"].shuffle(seed=42).select(range(5000))
    transformed_sample = sample.map(custom_transform, load_from_cache_file=False)
    augmented = concatenate_datasets([dataset["train"], transformed_sample])
    tokenized_augmented = augmented.map(tokenize_function, load_from_cache_file=False, batched=True)
    ...
    print("augmented_dataset_size", len(tokenized_augmented))
